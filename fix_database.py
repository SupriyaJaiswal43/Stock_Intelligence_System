"""
Run this ONCE to fix database.py:
...
All data is persisted to portfolio_store.json
"""
...
def _model_key(obj) -> str:
    ...
    if isinstance(obj, AIDecision):
        return "decisions"
    raise ValueError(f"Unknown model: {type(obj)}")
...
class _Query:
    ...
    def all(self):
        return [self._cls.from_dict(r) for r in self._results()]
...
# Write to database.py in same folder
target = os.path.join(os.path.dirname(os.path.abspath(__file__)), "database.py")
with open(target, "w", encoding="utf-8") as f:
    f.write(content)

print(f"✅ database.py fixed at: {target}")

# Verify
from database import SessionLocal, Portfolio, Alert, AIDecision
print("✅ SessionLocal  — OK")
print("✅ Portfolio     — OK")
print("✅ Alert         — OK")
print("✅ AIDecision    — OK")
print("\nAb dashboard restart karo: python dashboard.py")
